...
while (my $arg = shift @clargs) {
	...
	if($arg eq "")  {
	    $WARNING && print STDERR "WARNING: incorrect argument given to -k flag.\n";
	} else {
	    $eval_k = $arg;
	}
    } elsif($arg eq "-P") {
...
for my $i (@sorted_index) {
	...
	# $HitP[$i] = 1 - exp( - $eval_k * $hit_score[$i] ); # check K-A extreme value dist..  
	$logHitP[$i] = $eval_k * $hit_score[$i] ; # check K-A extreme value dist..  
	
	# log sum? or simply P product?

	$logSumContribution = $logHitP[$i] + $motifP;
    }
}
...
sub usage {
    ...
    print "\t\t\t\t[-k f]\tEval conversion constant (Karlin-Altschul db parameter)\n";
