...
sub count_dominance {
    ...
    while ( $dom_string =~ m/co/g )  {
	$co_dom++;
    }
    
    while ( $dom_string =~ m/cp/g ) {
	$co_dom++;
    }
	    
    while ( $dom_string =~ m/uks/g ) {
	$se_dom++;
    }
    
    while ( $dom_string =~ m/se/g ) {
	$se_dom++;
    }	    

    return ($co_dom, $se_dom);
}
...
#	    output sprintf("Column: %3d  Res: %s  Count: %3d  Dom: %s\n", $i, $res, $count{$res}, $dominance_by_res{$res});
	    ...
	    # keep tab of count of se resp. co
	    my ($nco,$nse) = count_dominance( $dominance_by_res{$res} );
	    
	    if( $nco == 0 || $nse == 0 ) {
		
		output sprintf("Column: %3d  Res: %s  Count: %3d  DKL: undef Dom: %s\n", $i, $res, $count{$res}, $dominance_by_res{$res});
	       $dkl_res = 0;
	   } else {
		my $N = $nco + $nse;
		my $p = $nse/$N;
		...
		$dkl_res = ($p - $q) * log( $p / $q ) ;
		$dkl_per_pos += $dkl_res;
		output sprintf("Column: %3d  Res: %s  Count: %3d  DKL_res: %.5f Dom: %s \n", $i, $res, $count{$res}, $dkl_res, $dominance_by_res{$res});
...
# oops on all gap.. please fix!
    ...
    output sprintf("DKL for column = %.5f\n", $dkl_per_pos);    
    output "\n";
}

output sprintf("DKL tot = %.5f\n", $dkl_tot);
