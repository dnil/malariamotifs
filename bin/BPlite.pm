# $Id: BPlite.pm,v 1.1.1.1 2009/09/23 10:46:03 daniel Exp $
##############################################################################
...
sub nextSbjct {
  ...
  while(defined ($_ = $self->_readline() ) ) {
    ...
    elsif ($_ =~ /^Histogram|^Searching|^Parameters|^\s+Database:|^\s+Posted date:|^Underlying/) {
	$self->_pushback($_); 
	last;
    }
    else                          {$def .= $_}
...
sub _fastForward {
    ...
    while(defined( $_ = $self->_readline() ) ) {
	if ($_ =~ /^Histogram|^Searching|^Parameters|^\s+Database:|^\s+Posted date:|^Underlying/) {
	    return 0;
	} elsif( $_ =~ /^>/ ) {
	    $self->_pushback($_);	
	    return 1;
