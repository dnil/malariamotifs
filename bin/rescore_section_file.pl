...
#	}
	...
	if ($arg eq "oneperisolate") {
	    $count_one_dominant_per_isolate = 1; # count only once per isolate per contig
	} else {
...
sub dhyper {
    ...
    if( $tries == 0 ) {
	$WARNING && print STDERR "\tProb undefined since count is 0.\n";
	next;
    }
	...
	$DEBUG && $WARNING && print STDERR "\tSucces count is 0; using fail count for hypergeometric prob.\n";
	$a = over($tries,$n);
	$b = $p**$m;
	$c = $q**$n;
	...
	$WARNING && print STDERR "\tOOPS - possibly too large numbers for dhyper: $hypgeo. Returning 1.\n"; 
	$hypgeo = 1;
    }
    
#    $precision_limit = 10^-16; # see perldoc perlnumber 
#    if ( $a < $precision_limit or $b < $precision_limit or $c $b < $precision_limit ) {
#	$WARNING && print STDERR "\tOOPS - numbers perhaps too small for any useful precision in dhyper: $hypgeo. Returning 1.\n"; 
#    }
