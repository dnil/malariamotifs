...
# v3: the strict mode was a decent improvement (in terms of specificity at least) over using the full sequence as db.
#     Lets try using the ultraconserved locations as guides for selecting the db rather than pressing a new for each column set.
#     This is biologically motivated (structurally and immunologically so) at least for surface antigens. The current 
#     N * peak_len is unsatisfying from a theoretical standpoint.
# 
# also add using a defined number of variable positions to still count as an ultraconserved (number of 0s to bridge between such 1s for peak formation..)
...
my (@ultracons_peak_center) = find_peak(@peak); # sets used..
my $n_peaks = @ultracons_peak_center;
...
foreach my $seed_id ( @motifs ) {
	...
	$DEBUG && output "DEBUG: Calling dhyper for seed $seed_id ($actual_members actual members) with M=$se_dominants, N=$co_dominants, m=$se_dominants_in_motif, n=$co_dominants_in_motif.\n";
...
#		output  "Found indicator peak between $peak_start and $peak_end.\n";
		...
		push @peak_center, int( ($peak_end - $peak_start) /2);
#		$n_peaks++;
...
#    my %seq_rel_region_end;
    ...
    if ( $strict_section == 1 && $has_established_regions == 0) {       
	
	establish_region_databases();

    } elsif($has_established_regions == 1)  {

#	if($region_formatting_done == 0) {
	# format the regions
	
	my $minimal_peak_region = $min_blastp_len;
	my $last_peak = 0;

	for (my $i = 0; $i < @ultracons_peak_center; $i++) {
	    
	    $seed_region_start = $last_peak;
	    if($seed_region_end - $seed_region_start < $minimal_peak_region ) {
		$DEBUG && print "semistrict: region ($seed_region_start,$seed_region_end) is rather short. Keeping start and moving on to next end making a larger region.\n";
		next;
	    }
...
sub select_correct_region_database {
    
    my $seed_start = shift;
    my $seed_end = shift;     # used only make note of seeds not being actually contained within one region..


    my $region_db_name = "";

    for (my $i = 0; $i < @region_start; $i++) { 
	
	if ($seed_start < $region_start[$i]) {
	    # seed starts in region $i.
	    $region_db_name = "";
	    
	    # check if it goes out over reg start..
	    if ($seed_end > $region_start[$i]) {

x	    }

	} elsif( $i == (@region_start-1) ) {
	    # so, seed was not found to start  any of the previous regions. Must be in the last region then.

	    
	}
	
    
    return $region_db_name;

}

sub establish_region_databases {
   

    for (my $i = 0; $i < @region_start; $i++) { 

    my $region_db_name = $db_file"_".$region_id;
